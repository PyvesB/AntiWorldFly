...
public class RegisterAntiElytraFlag {

    private final AntiWorldFly plugin;

    public RegisterAntiElytraFlag(AntiWorldFly awf) {
        this.plugin = awf;
    }
    ...
    public void register() {
        if (plugin.getServer().getPluginManager().getPlugin("WorldGuard") != null) {
            ...
            try {
                ...
                StateFlag flag = new StateFlag(plugin.getAntiElytraFlag(), true);
                ...
                Flag<?> existing = registry.get(plugin.getAntiElytraFlag());
                if (existing instanceof StateFlag) {
                    antiFlyFlag = (StateFlag) existing;
                } else {
                    // types don't match - this is bad news! some other plugin conflicts with you
                    // hopefully this never actually happens
                    plugin.getLogger().warning("Multiple plugins are registering the flag \"" + plugin.getAntiElytraFlag() + "\"");
                }
            }
        }
    }
